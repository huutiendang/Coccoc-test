/*
Compile:
    g++ main.cpp -o <exe_file_name> 
Run:
    ./<exe_file_name> <input_file_name> <output_file_name> <limit_memory>

Assume input_file and output_file existed.   
*/

#include <iostream>
#include <string>
#include <vector>
#include <fstream>
#include <algorithm>
#include <queue>

using namespace std;

char fileName[10000]; 

#define limited_memory 8000000000 //8GB
#define r_int register int

struct node {  
    string element; 
    int index; 
}; 

class Comparator { 
public: 
    int operator() (const node& node1, const node& node2) { 
        return (node1.element.compare(node2.element) > 0);
    } 
}; 

template<class Iter>
void mergeSort(Iter first, Iter last) { 
    if (last - first > 1) { 
        Iter middle = first + (last - first) / 2;
        mergeSort(first, middle);
        mergeSort(middle, last);
  		inplace_merge(first, middle, last);
	} 
} 

void itoa(int num, char *arr, int &cnt) {
 	if (num <= 0 && cnt > 0) return;
 	if (num == 0 && cnt == 0) {
  		arr[cnt] = '0';
  		arr[cnt+1] = '\0';
  		return;
 	}
 	if (num > 0) {
  		itoa(num/10, arr, cnt);
  		arr[cnt] = num%10 + '0';
  		++cnt;
  		arr[cnt] = '\0';
 	}
}

void create_Initial_Runs(string input_file, string output_file, unsigned long long mem_limit, int &num_ways) { 
 	if (mem_limit > limited_memory) mem_limit = limited_memory;
 	ifstream f_in;
 	f_in.open(input_file);

 	//Determine file size
 	streampos begin = f_in.tellg();
 	f_in.seekg(0, ios::end);
 	streampos end = f_in.tellg();
 	unsigned long long size = end-begin;
 	if (size <= mem_limit)
  		num_ways = 1;
 	else
  		num_ways = (size/mem_limit) + 1;
 	f_in.clear();
 	f_in.seekg(0, ios::beg);

 	if (num_ways > 1) {
  		ofstream *f_out = new ofstream[num_ways];
  		for (r_int i = 0; i < num_ways; i++) { 
   			int cnt = 0;
   			itoa(i, fileName, cnt);
   			f_out[i].open(fileName);
  		} 
  
  		bool more_input = true; 
  		int next_output_file = 0; 
  
  		while (more_input) { 
  			vector<string> *arr = new vector<string>;
   			size_t countByte = 0;
   			while (countByte < mem_limit) {
    			if (f_in.eof()) { 
     				more_input = false; 
     				break; 
    			} 
    			if (more_input) {
     				string *temp = new string();
     				getline(f_in, *temp);
     				arr->push_back(*temp);
     				countByte += temp->size();
     				delete temp;
    			}
   			}
   			mergeSort(arr->begin(), arr->end()); 
   			f_out[next_output_file].clear();
   			for (r_int j = 0; j < arr->size(); j++) {
    			if (arr->at(j).size()) {
     				f_out[next_output_file]<<arr->at(j)<<"\n";
    			}
			}
			delete arr;	
   			++next_output_file; 
  		}	 
  		for (r_int i = 0; i < num_ways; i++)
   			f_out[i].close(); 
   			
		f_in.close();
		return;
 	}
 	else if (num_ways == 1) {
 		vector<string> *arr = new vector<string>;
  		while (true) { 
   			if (f_in.eof()) { 
    			break; 
   			}	 
   			string *temp = new string();
   			getline(f_in, *temp);
   			arr->push_back(*temp);
   			delete temp;
  		}
  		mergeSort(arr->begin(), arr->end()); 
  		ofstream f_out(output_file);
  		f_out.clear();
  		for (r_int j = 0; j < arr->size(); j++) {
   			if (arr->at(j).size()) {
    			f_out<<arr->at(j)<<"\n";
   			}
  		}
  		
  		delete arr;
  		f_in.close();
  		f_out.close();
  		return;
 	}
} 

bool isEmptyFile(ifstream &file) {
 	return file.peek() == ifstream::traits_type::eof();
}

void mergeFiles(string output_file, int num_ways) { 
 	if (num_ways == 1) return;
 	ifstream *f_in = new ifstream[num_ways];
    for (r_int i = 0; i < num_ways; i++) { 
  		int cnt = 0;
  		itoa(i, fileName, cnt);
  		f_in[i].open(fileName);
    } 
 	ofstream f_out(output_file);
 	f_out.clear();

 	priority_queue<node, vector<node>, Comparator> min_heap; 
  
    for (r_int i = 0; i < num_ways; i++) { 
	  	if (!isEmptyFile(f_in[i])) {
	  		node *temp = new node();
   			getline(f_in[i], temp->element);
   			temp->index = i;
   			min_heap.push(*temp);
   			delete temp;
  		}
    } 

    int count = 0; 
  
 	while (count != num_ways) { 
 		node *root = new node();
  		*root = min_heap.top();
  		min_heap.pop();
  		if (root->element.size())
   			f_out<<root->element<<"\n";

  		if(f_in[root->index].eof()) {
            ++count; 
   			continue;
  		}
  		else 
   			getline(f_in[root->index], root->element);
   		min_heap.push(*root);
   		delete root;
    } 

 	for (r_int i = 0; i < num_ways; i++) {
  		f_in[i].close(); 
  		int cnt = 0;
  		itoa(i, fileName, cnt); 
  		remove(fileName);
 	}
 	f_out.close(); 
} 

void external_MergeSort(string input_file, string output_file, unsigned long long mem_limit) { 
 	int num_ways;
    create_Initial_Runs(input_file, output_file, mem_limit, num_ways); 
 	mergeFiles(output_file, num_ways); 
} 

unsigned long long isValidMem(string mem) {
 	int len = mem.size();
    //cout <<len; 
 	unsigned long long limit_mem = 0;
 	for (r_int i = 0; i < len; i++) {
  		limit_mem = limit_mem*10 + (mem[i] - '0');
 	}
 	if (limit_mem > limited_memory) {
 		cout <<"Too large! Limited memory is 8000000000"<<endl;
 		return false;
	}
 	return limit_mem;
}

int main(int argc, char **argv) { 
 	
    external_MergeSort(argv[1], argv[2], isValidMem(argv[3]));
 	cout << "Sorted!"<<endl;
    return 0; 
}